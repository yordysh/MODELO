...
function generar_filas_tabla_dinamica() {
    ...
    $("#agregarFila").on("click", function () {
        ...
        var nuevaFila = "<tr>" +
            "<td>" + "<input type='hidden' class='form-control' name='contadorFilas[]' value=" + contadorFilas + ">" + contadorFilas + "</td>" +
            ...
            if (response.length > 0) {
                ...
                for (var i = 0; i < response.length; i++) {
                    ...
                    <tr>
                        ...
                        <td><button onclick="generar_maqueta_insumos('${item.COD_AVANCE_INSUMOS}')" class="btn btn-success"><i class='fas fa-plus'></i></button></td>
                    </tr>`;
...
function generar_maqueta_insumos(cod_avance_insumos) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Limpiar checkbox
            $('#tablaEvaluacionSensorial input[type="checkbox"]').prop('checked', true);

            // Limpiar campos de texto y convertir a mayúsculas (excepto la fecha)
            $('#txt_acetado_rechazado, #txt_analista').val('').on('input', function () {
            ...
            success: function (response) {
                ...
                if (response.length > 0) {
                    ...
                    for (let i = 0; i < response.length; i += 2) {
                        ...
                        doc.text("FECHA: " + response[0].NOMBRE_MES + " " + response[0].ANIO, 211, 17); //campo para agregar la version reemplazar
                        ...
                        if (item1) {
                            ...
                            for (let i = 0; i < 15; i++) {
                                ...
                                if (i < item1.items.length) {
                                    ...
                                    if (item1.items[i].OBSERVACIONES !== null && item1.items[i].OBSERVACIONES !== undefined) {
                                    ...
                                    if (item1.items[i].ACCION_CORRECTIVA !== null && item1.items[i].ACCION_CORRECTIVA !== undefined) {
                        ...
                        if (item2) {
                            ...
                            for (let i = 0; i < 15; i++) {
                                ...
                                if (i < item2.items.length) {
                                    ...
                                    if (item2.items[i].OBSERVACIONES !== null && item2.items[i].OBSERVACIONES !== undefined) {
                                    ...
                                    if (item2.items[i].ACCION_CORRECTIVA !== null && item2.items[i].ACCION_CORRECTIVA !== undefined) {
            ...
            success: function (response) {
                ...
                if (response.length > 0) {
                    ...
                    for (let i = 0; i < response.length; i++) {
                        ...
                        if (itemsK[0].OBSERVACION_SE !== null && itemsK[0].OBSERVACION_SE !== undefined) {
                        ...
                        if (itemsK[0].ACCION_CORRECTIVA_SE !== null && itemsK[0].ACCION_CORRECTIVA_SE !== undefined) {
