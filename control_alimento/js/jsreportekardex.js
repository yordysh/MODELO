   ...
   $.ajax({
    ...
    success:function(e) {
        try {
            ...
            console.log(obj['d']);
            tabla = '';
            if(obj['m'] != ''){Mensaje1(obj['m'] ,"info"); return;}
            if(obj['c'] > 0){
                $.each(obj['d'],function(i,t) {
                    console.log(i);
                    tabla += '<tr class="trcambio">'+
                            '<td title="producto">'+t[2]+"-"+t[1]+'</td>'+
                            '<td title="lote">'+t[3]+'</td>'+
                            '<td title="fecha"> '+t[8]+'</td>'+
                            '<td title="descripcion">'+t[4]+'</td>'+
                            '<td title="ingreso">'+t[5]+'</td>'+
                            '<td title="salida">'+t[6]+'</td>'+
                            '<td title="saldo">'+t[7]+'</td>'+
                            '</tr>';
                            if(obj['c'] == (i+1)){
                                tabla += '<tr class="trcambio">'+
           ...
           try {
             obj = JSON.parse(e);
                if(obj['c'] > 0){
                    ...
                    $('#slcproducto').append($("<option value="+item[0]+">"+item[1]+"</option>" ));  
                    });
                }
           } catch (error) {
                console.log(e);
           ...
           try {
             ...
             console.log(obj);
                if(obj['c'] > 0){
                    ...
                    .val('')
                    $.each(obj['d'],function(i,item) {
                    $('#slclote').append($("<option value="+item[0]+">"+item[0]+"</option>" ));  
                    });
                }else{
