...
function generar_pdfControlBachada_2() {
    $(document).on('click', 'button.generar_pdf_reporte_control_bachada2', function () {
                    ...
                    for (const codPersonal in bachadasPorPersonal) {
                        if (bachadasPorPersonal.hasOwnProperty(codPersonal)) {
                            ...
                            for (let i = 0; i < bachadas.length; i++) {
                                ...
                                doc.text(formatDateAnio(currentItem.FECHA_MEZCLADO), 8, s - 2); // Columna de Fecha
                                ...
                                doc.text((+currentItem.MEZCLA_SELECT_BOL_INCO).toFixed(3), 135, s - 2);

                                //doc.text(currentItem.MEZCLA_SOBRANTE, 153, s - 2); // Columna de 
                                doc.text((+currentItem.MEZCLA_SOBRANTE).toFixed(3), 153, s - 2); // Columna de 
                                ...
                                doc.text((+currentItem.PESO_TOTAL_BOL_SOBRANTE).toFixed(3), 220, s - 2); // Columna de 

                                doc.text(currentItem.LOTE, 236, s - 2); // Columna de 
                                doc.text(formatDateAnio(currentItem.FECHA_PRODUCCION), 246, s - 2); // Columna de 
                                doc.text(formatDateAnio(currentItem.FECHA_VENCIMIENTO), 258, s - 2); // Columna de 
...
function traer_datos_productos_ylotesPendientes() { //DATOS PARA LA BUSQUEDA Y GENERAR LA TABLA
    var productos = [];
    var lotes = [];
    ...
    $('.generar_datos_busqueda').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.length > 0) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        for (var i = 0; i < response.length; i++) {
                            ...
                            var newRow = `
                    <tr>
                        <td class="id_kard" data-id_kard="${item.ID}" data-estado_fila="${item.ESTADO}">${i + 1}</td>
                        ...
                        <td><button onclick="generar_maqueta_bachada('${item.ID}')" class="btn btn-success btn_agregar_bachada_genera"><i class='fas fa-plus'></i></button></td>
                    </tr>`;
                ...
                $('#tablaDatosPendientes_Bachadas tbody').find('tr').each(function () {
...
function limpiar_datos_input_bachada() {
    $('#codigo_id_kardex, #peso_neto_producto, #nombre_encargado, #codigo_encargado, #producto_bachada, #fecha_mezclado, #numero_bachada, #peso_total_obtenido, #cantidad_de_bolsas').val('');

    $('#cantidad_programada_unidades, #peso_estimado_kg, #bolsas_seleccionadas_cantidad, #bolsas_seleccionadas_peso_total_kg, #bolsa_incompleta_mezcla_seleccionada').val('');

    $('#mezcla_sobrante, #mezcla_total_envasar, #cantidad_estimada_unidad, #bolsas_sobrantes_cantidad, #bolsas_sobrante_peso_total, #lote_bachada').val('');

    $('#fecha_de_produccion, #fecha_de_vencimiento, #observaciones_envasado, #acc_correctiva_envasado').val('');
}
...
function calcularTotalPesoEstimadoKg() {
    ...
    var cantidad_programada_unidades = parseFloat($('#cantidad_programada_unidades').val()) || 0;
...
function calcularTotales() {
    ...
    var bolsas_seleccionadas_peso_total_kg = parseFloat($('#bolsas_seleccionadas_peso_total_kg').val()) || 0;
    var bolsa_incompleta_mezcla_seleccionada = parseFloat($('#bolsa_incompleta_mezcla_seleccionada').val()) || 0;
    var bachada_anterior = parseFloat($('#bachada_anterior').val()) || 0;

    totalSobrante = (peso_total_obtenido + bachada_anterior) - (bolsas_seleccionadas_peso_total_kg + bolsa_incompleta_mezcla_seleccionada);
    totalEnvasar = bolsas_seleccionadas_peso_total_kg + bolsa_incompleta_mezcla_seleccionada;
    ...
    $('#cantidad_estimada_unidad').val(parseInt(cantidad_estimada_unidades));
    console.log('nm 01');
}
...
function calcularBolsasSobrantes() {
    ...
    var bolsas_seleccionadas_cantidad = parseFloat($('#bolsas_seleccionadas_cantidad').val()) || 0;

    totalCantidadBolsasSobrante = cantidad_total_bolsas - bolsas_seleccionadas_cantidad;
...
function traer_datos_productos_ylotesReporte() { //DATOS PARA GENERAR EL REPORTE
    var productos = [];
    var lotes = [];
